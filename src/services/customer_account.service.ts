...
class AccountService {
    ...
    async syncPhoneAccount(payload: { uid: string; phone: string }) {
        ...
        if (!account) {
            ...
            account = await prisma.customerAccount.create({
                data: {
                    ...
                    accountIdentifier: payload.uid, // UID từ Supabase
    ...
    // 3. SET PASSWORD (Đã sửa: dùng phone và uid thay vì customerId)
    async setPassword(phoneInput: string, uid: string, newPassword: string) {
        // [QUAN TRỌNG]: Đảm bảo UID khớp với tài khoản đã được Supabase verify
        ...
        // Đảm bảo SĐT trong CustomerAccount khớp với SĐT đang dùng
        if (account.customer.phone !== normalizedPhone) {
        ...
        // Sau khi đặt mật khẩu, trả về token để client đăng nhập luôn
        const tokens = this.createTokenPair(account.customerId);
